...
async function syncFromFirestore() {
  ...
  try {
    ...
    // Push any pending local edits first so the cloud becomes the source of truth.
    ...
    records = remoteRecords.filter(r => !r.deleted);
    saveLocal();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.addEventListener('online', async () => {
    ...
    if (currentUser && fbAvailable) {
      ...
      // Was in offline-session mode; re-enable Firebase if possible
