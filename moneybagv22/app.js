...
/* Measures the balance card and bank card heights so the card-stack
   can size the peek strip and the expanded reveal correctly. */
function measureCardStack() {
  const cardStack = document.getElementById('card-stack');
  const balanceCard = document.getElementById('balance-card');
  const bankCardEl = document.getElementById('bank-card');
  if (!cardStack || !balanceCard || !bankCardEl) return;
  cardStack.style.setProperty('--balance-card-h', balanceCard.offsetHeight + 'px');
