...
function showTab(tab) {
  ...
  if (tab === 'examlinks') loadExamLinks();
}
        ...
        return `
          <button class="session-card" onclick="openSessionDetail('${esc(s.subject)}','${s.dateKey}')">
            ...
            <div class="session-card-stats">
              ...
              ${absentCount > 0 ? `<span class="session-stat session-stat-absent"><i class="fa-solid fa-user-xmark"></i> ${absentCount} absent</span>` : ''}
            </div>
...
function renderSessionDetail() {
  ...
  const countEl = document.getElementById('resultCount');
  const totalAbsent = getAbsentees(session).length;
  countEl.textContent = `${session.entries.length} result${session.entries.length !== 1 ? 's' : ''}` +
    (totalAbsent > 0 ? ` · ${totalAbsent} active student${totalAbsent !== 1 ? 's' : ''} absent` : '');
...
/* ════════════════════════════════════════════════════════════════
   EXAM LINKS — managed list stored in Firestore `exam_links` collection
   Each doc: { slug: 'chemistry', label: 'Chemistry — Chapter 3' (optional) }
   Public exam URL pattern: index.html?exam=<slug>
   Question files are fetched directly from:
   https://antargfx.github.io/codelab/examv12/questions/<slug>.json
════════════════════════════════════════════════════════════════ */

const QUESTIONS_BASE_URL = 'https://antargfx.github.io/codelab/examv12/questions/';
...
async function loadExamLinks() {
  ...
  const countEl = document.getElementById('examLinksCount');
  container.innerHTML = '<div class="empty-row" style="padding:40px;text-align:center"><div class="spinner-sm"></div> Loading exam links…</div>';
  countEl.textContent = 'Loading…';

  try {
    const snap = await db.collection('exam_links').orderBy('slug').get();
    allExamLinks = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    await renderExamLinks(allExamLinks);
  } catch (err) {
    console.error('[Admin] loadExamLinks error:', err);
    container.innerHTML = `<div class="empty-row" style="padding:40px;text-align:center;color:var(--clr-danger)">
      <i class="fa-solid fa-triangle-exclamation"></i> Failed to load exam links: ${esc(err.message)}
    </div>`;
    countEl.textContent = '';
  }
}

async function renderExamLinks(links) {
  const container = document.getElementById('examLinksContainer');
  const countEl = document.getElementById('examLinksCount');
  countEl.textContent = `${links.length} exam link${links.length !== 1 ? 's' : ''}`;

  if (links.length === 0) {
    container.innerHTML = '<div class="empty-row" style="padding:40px;text-align:center"><i class="fa-solid fa-inbox"></i> No exam links yet. Add one above.</div>';
    return;
  }

  const baseUrl = window.location.origin + window.location.pathname.replace('admin.html', '');

  // Try to fetch the live subject name from each question file (best-effort, non-blocking on failure)
  const enriched = await Promise.all(links.map(async (link) => {
    let liveSubject = null;
    try {
      const qRes = await fetch(QUESTIONS_BASE_URL + encodeURIComponent(link.slug) + '.json?_=' + Date.now(), { cache: 'no-store' });
      if (qRes.ok) {
        const qData = await qRes.json();
        liveSubject = qData.subject || null;
      }
    } catch (_) { /* ignore — fall back to stored label/slug */ }
    return { ...link, liveSubject };
  }));

  const examLink = slug => `${baseUrl}index.html?exam=${encodeURIComponent(slug)}`;

  container.innerHTML = `
    <div class="exam-links-grid">
      ${enriched.map(({ id, slug, label, liveSubject }) => {
        const displayLabel = liveSubject || label || slug;
        const link = examLink(slug);
        const fileMissing = liveSubject === null;
        return `
          <div class="exam-link-card">
            <div class="exam-link-top">
              <div class="exam-link-icon"><i class="fa-solid fa-book-open"></i></div>
              <div class="exam-link-info">
                <p class="exam-link-subject">${esc(displayLabel)}</p>
                <span class="exam-link-id">File: ${esc(slug)}.json${fileMissing ? ' <span class="exam-link-warn">(not found)</span>' : ''}</span>
              </div>
              <button class="action-btn delete" title="Remove" onclick="openDeleteExamModal('${esc(id)}', '${esc(slug)}')">
                <i class="fa-solid fa-trash"></i>
              </button>
            </div>
            <div class="exam-link-url-row">
              <span class="exam-link-url" id="examUrl_${esc(id)}">${esc(link)}</span>
            </div>
            <div class="exam-link-actions">
              <button class="btn btn-primary btn-sm" onclick="copyExamLink('${esc(link)}', '${esc(id)}')">
                <i class="fa-solid fa-copy" id="copyIcon_${esc(id)}"></i> Copy Link
              </button>
...
async function addExamLink() {
  const slugInput  = document.getElementById('newExamSlug');
  const labelInput = document.getElementById('newExamLabel');
  const alertEl    = document.getElementById('addExamAlert');
  ...
  if (!slug) {
    alertEl.textContent = 'Please enter the exam file name (e.g. chemistry).';
    alertEl.classList.add('show');
    return;
  }
  if (!/^[a-zA-Z0-9_-]+$/.test(slug)) {
    alertEl.textContent = 'File name can only contain letters, numbers, hyphens, and underscores.';
    alertEl.classList.add('show');
    return;
  }
  if (allExamLinks.some(l => l.slug === slug)) {
    alertEl.textContent = `An exam link for "${slug}" already exists.`;
    alertEl.classList.add('show');
    return;
  }
  ...
  try {
    await db.collection('exam_links').add({
      slug,
      label: label || null,
    ...
    slugInput.value = '';
    labelInput.value = '';
    showToast('Exam link added.', 'success');
    await loadExamLinks();
  } catch (err) {
    console.error('[Admin] addExamLink error:', err);
    alertEl.textContent = 'Failed to add exam link: ' + err.message;
    alertEl.classList.add('show');
  } finally {
    btn.disabled = false;
    btn.innerHTML = '<i class="fa-solid fa-plus"></i> Add Exam';
  }
}

function openDeleteExamModal(id, slug) {
  pendingDeleteExamId = id;
  document.getElementById('deleteExamModalMsg').textContent =
    `Remove the exam link for "${slug}"? Students will no longer be able to use this link.`;
  document.getElementById('deleteExamModal').classList.add('open');
}

function closeDeleteExamModal() {
  pendingDeleteExamId = null;
  document.getElementById('deleteExamModal').classList.remove('open');
}
...
async function confirmDeleteExam() {
  ...
  try {
    await db.collection('exam_links').doc(pendingDeleteExamId).delete();
    showToast('Exam link removed.', 'info');
    closeDeleteExamModal();
    await loadExamLinks();
  } catch (err) {
    console.error('[Admin] deleteExamLink error:', err);
    showToast('Remove failed: ' + err.message, 'error');
  } finally {
    btn.disabled = false;
    btn.innerHTML = '<i class="fa-solid fa-trash"></i> Yes, Remove';
  }
}

function copyExamLink(link, id) {
  navigator.clipboard.writeText(link).then(() => {
    const icon = document.getElementById('copyIcon_' + id);
