/**
 * script.js — ExamPrep v14
 *
 * Changes from v12:
 *  - Dropped the questions.json index/lookup table entirely.
 *  - Exam is now selected via ?exam=<filename> in the URL, e.g. ?exam=chemistry
 *    or ?exam=bangla01. The matching question file is fetched directly from
 *    GitHub Pages: https://antargfx.github.io/codelab/examv12/questions/<exam>.json
 *  - localStorage state is now keyed per exam slug, so different exams never
 *    collide or restore each other's in-progress answers.
 *  - Saved results now include the exam slug (examId field, reused for
 *    backward compatibility with existing Firestore documents) for traceability.
 *
...
const LS_KEY_PREFIX = 'examprep_state_v3_';

/** Base folder where all subject question JSON files live. */
const QUESTIONS_BASE_URL = 'https://antargfx.github.io/codelab/examv12/questions/';

/** Per-exam localStorage key — keeps separate exams (different exam slug) from
 *  ever colliding or restoring each other's in-progress state. */
function getLsKey() {
  return LS_KEY_PREFIX + (exam.examId || 'default');
}
...
async function loadExamData() {
  ...
  // The exam to load is selected via ?exam=chemistry in the URL.
  // This maps directly to a filename in the questions folder: chemistry.json
  const examSlugRaw = new URLSearchParams(window.location.search).get('exam');
  const examSlug = examSlugRaw ? examSlugRaw.trim() : '';

  if (!examSlug) {
    showLoadError('No exam selected. Open this page using a valid exam link, e.g. ?exam=chemistry');
    return;
  }

  // Basic sanity check — only allow safe filename characters.
  if (!/^[a-zA-Z0-9_-]+$/.test(examSlug)) {
    showLoadError('Exam not found. Please contact the admin for the correct exam link.');
    return;
  }

  try {
    const examUrl = QUESTIONS_BASE_URL + examSlug + '.json';
    const dataRes = await fetch(examUrl + '?_=' + Date.now(), { cache: 'no-store' });

    if (!dataRes.ok) {
      if (dataRes.status === 404) {
        throw new Error('Exam not found. Please contact the admin for the correct exam link.');
      }
      throw new Error(`HTTP ${dataRes.status}: Could not fetch questions for this exam.`);
    }

    const data = await dataRes.json();

    if (!data.questions || !Array.isArray(data.questions) || data.questions.length === 0) {
      throw new Error('This exam\'s question file is missing the "questions" array or it is empty.');
    }

    exam.examId           = examSlug;
    exam.data              = data;
...
async function handleLogin() {
  ...
  // Only students with an "active" status may attend the exam
...
function hideInactive() {
  document.getElementById('inactiveAlert').style.display = 'none';
  document.getElementById('startBtn').style.display = '';
}
...
// Reset not-found / inactive banners when user edits the mobile field
document.getElementById('studentMobile').addEventListener('input', () => {
  hideNotFound();
  hideInactive();
});
...
function restoreFromLocalStorage() {
  try {
    ...
    if (state.examId !== exam.examId) return false;
