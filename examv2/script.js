/**
 * script.js — ExamPrep v2
 *
 * Changes from v1:
 *  - All questions rendered on one scrollable page (no prev/next navigation)
 *  - Answer review section on result screen (correct / wrong / skipped with right answers)
 *  - Font Awesome icons (no emoji in JS)
...
const LS_KEY = 'examprep_state_v2';
...
function initTheme() {
  // Light theme is default (already set in <html data-theme="light">)
...
async function submitExam(autoSubmit = false) {
  ...
  saveStatusEl.innerHTML = saved
    ...
    : '<i class="fa-solid fa-triangle-exclamation" style="color:var(--clr-warning)"></i> Could not save remotely (check Firebase config).';
...
function displayResult(result) {
  ...
  if (passed) {
    resultIcon.className = 'result-icon pass-icon';
    iconEl.className     = 'fa-solid fa-trophy';
  } else {
    resultIcon.className = 'result-icon fail-icon';
...
function renderAnswerReview() {
  ...
  exam.questions.forEach((q, i) => {
    ...
    if (isCorrect) {
      ...
      statusIcon = '<i class="fa-regular fa-circle-dot"></i>';
    }
    ...
    Object.entries(q.options).forEach(([key, text]) => {
      ...
      if (isCorrectOpt) {
        ...
        tagHTML   = '<span class="review-option-tag">Correct</span>';
      } else if (isUserWrong) {
        optClass += ' opt-wrong-user';
        tagHTML   = '<span class="review-option-tag">Your Answer</span>';
      }
...
function restoreFromLocalStorage() {
  try {
    ...
    console.log('[ExamPrep] Restored previous session.');
    return true;
  } catch (e) {
