...
/* ══════════════ FIREBASE INIT ══════════════ */
let db, auth, fbAvailable = false;

(function initFirebase() {
  try {
    const fb = window.__firebase;
    if (!fb || !fb.firebaseConfig.apiKey) {
...
function getCategoryEmoji(cat) {
  const map = { Food:'🍔', Shopping:'🛍', Rent:'🏠', Fuel:'⛽', Health:'💊', Education:'📚', Entertainment:'🎬', Salary:'💼', Freelance:'💻', Investment:'📈', Other:'📦' };
  return map[cat] || '📦';
}
...
async function afterLogin() {
  loadLocal();
  updateProfileUI();
  if (fbAvailable) await syncFromFirestore();
  showApp();
  navigate('dashboard');
...
async function syncFromFirestore() {
  ...
  try {
    ...
    const fb = window.__firebase;
    const col = fb.collection(db, 'users', currentUser.uid, 'moneybagRecords');
...
async function flushPendingSync() {
  ...
  for (const op of toSync) {
    try {
      if (op.type === 'upsert') {
        const ref = fb.doc(db, 'users', currentUser.uid, 'moneybagRecords', op.record.id);
        await fb.setDoc(ref, op.record);
      } else if (op.type === 'delete') {
        const ref = fb.doc(db, 'users', currentUser.uid, 'moneybagRecords', op.id);
...
function txnHTML(r) {
  ...
  return `
    <div class="txn-item" data-id="${r.id}">
      <div class="txn-cat-icon type-${r.type}">${getCategoryEmoji(r.category)}</div>
      <div class="txn-info">
...
function renderInsights(stats) {
  ...
  if (stats.weeklySpent > 0) {
    ...
    if (lastWeekSpent > 0) {
      ...
      if (diff > 0) tips.push({ icon: '⬆️', text: `You spent ${fmt(diff)} more this week than last week.` });
      else if (diff < 0) tips.push({ icon: '⬇️', text: `Great! You spent ${fmt(-diff)} less this week than last week.` });
    }
  }

  if (stats.monthlySavings > 0) {
    tips.push({ icon: '💰', text: `Your monthly savings so far: ${fmt(stats.monthlySavings)}. Keep it up!` });
  }
  ...
  if (topCat) tips.push({ icon: '📊', text: `Biggest expense category: ${topCat[0]} (${fmt(topCat[1])})` });
  ...
  if (isFinite(oldest)) {
    ...
    tips.push({ icon: '📅', text: `Average daily expense: ${fmt(stats.totalSpent / days)}` });
  }
...
function renderSmartReview(stats) {
  ...
  if (lastWeekSpent) {
    ...
    tips.push({ icon: d > 0 ? '📈' : '📉', text: d > 0 ? `You spent ${fmt(d)} more this week than last week` : `You spent ${fmt(-d)} less this week than last week` });
  }
  if (lastMonthSpent) {
    const d = stats.monthlySpent - lastMonthSpent;
    tips.push({ icon: d > 0 ? '⚠️' : '✅', text: d > 0 ? `Monthly expenses up by ${fmt(d)} vs last month` : `Monthly expenses down by ${fmt(-d)} vs last month` });
  }
  if (stats.monthlySavings > 0) {
    tips.push({ icon: '💪', text: `Your savings improved this month: ${fmt(stats.monthlySavings)} saved` });
  }

  const cats = {};
  records.filter(r => !r.deleted && r.type === 'spend').forEach(r => { cats[r.category] = (cats[r.category]||0)+Number(r.amount); });
  const top = Object.entries(cats).sort((a,b) => b[1]-a[1])[0];
  if (top) tips.push({ icon: '🏆', text: `Biggest expense category: ${top[0]} (${fmt(top[1])})` });

  if (!tips.length) tips.push({ icon: '👋', text: 'Add more transactions to see your smart financial review!' });

  el.innerHTML = tips.map(t => `
    <div class="review-item">
      <span class="review-icon">${t.icon}</span>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btn-signup')?.addEventListener('click', async () => {
    ...
    try {
      showLoading();
      const { createUserWithEmailAndPassword, updateProfile } = window.__firebase;
  ...
  document.getElementById('btn-theme-toggle')?.addEventListener('click', () => {
    darkMode = !darkMode;
    localStorage.setItem('mb_dark', darkMode);
    applyTheme();
