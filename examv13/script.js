/**
 * script.js — ExamPrep v12
 *
 * Changes from v11:
 *  - Multi-exam support via ?examid=1520 in the URL.
 *    questions.json is now an index: { "1520": "<url to question file>", ... }
 *    The matching question file is fetched in a second step and used as before.
 *  - localStorage state is now keyed per examid, so different exams never
 *    collide or restore each other's in-progress answers.
 *  - Saved results now also include the examId for traceability.
 *
...
/** Per-exam localStorage key — keeps separate exams (different examid) from
...
async function loadExamData() {
  ...
  // The exam to load is selected via ?examid=1520 in the URL.
  const examId = new URLSearchParams(window.location.search).get('examid');

  if (!examId) {
    showLoadError('No exam selected. Open this page using a valid exam link, e.g. ?examid=1520');
    return;
  }

  try {
    // ── Step 1: fetch the exam index — maps examid → question-file URL ──
    const indexRes = await fetch('questions.json?_=' + Date.now(), { cache: 'no-store' });
    if (!indexRes.ok) throw new Error(`HTTP ${indexRes.status}: Could not fetch the exam index (questions.json).`);

    const examIndex = await indexRes.json();
    const examUrl = examIndex[examId];

    if (!examUrl) {
      throw new Error('Exam not found. Please contact the admin for the correct exam link.');
    }

    // ── Step 2: fetch the actual question set this examid points to ──
    const sep = examUrl.includes('?') ? '&' : '?';
    const dataRes = await fetch(examUrl + sep + '_=' + Date.now(), { cache: 'no-store' });
    if (!dataRes.ok) throw new Error(`HTTP ${dataRes.status}: Could not fetch questions for this exam.`);
