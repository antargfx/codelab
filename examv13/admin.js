...
async function loadExamLinks() {
  ...
  try {
    ...
    const examIndex = await res.json();

    const examIds = Object.keys(examIndex);
    if (examIds.length === 0) {
      container.innerHTML = '<p class="empty-row" style="padding:40px;text-align:center;color:var(--clr-text-muted)">No exams found in questions.json.</p>';
      return;
    }

    // Fetch subject name from each question file
    const examItems = await Promise.all(examIds.map(async (id) => {
      const url = examIndex[id];
      try {
        const sep = url.includes('?') ? '&' : '?';
        const qRes = await fetch(url + sep + '_=' + Date.now(), { cache: 'no-store' });
        if (!qRes.ok) return { id, url, subject: null, error: `HTTP ${qRes.status}` };
        const data = await qRes.json();
        return { id, url, subject: data.subject || null, error: null };
      } catch (e) {
        return { id, url, subject: null, error: e.message };
      }
    }));

    // Build the base URL for exam links (same origin + path, without query string)
    const base = window.location.href.replace(/\/admin\.html.*$/, '/index.html');

    container.innerHTML = examItems.map(item => {
      const examLink = `${base}?examid=${item.id}`;
      const label = item.subject ? item.subject : `Exam ${item.id}`;
      const subText = item.error
        ? `<span style="color:var(--clr-danger);font-size:0.8rem"><i class="fa-solid fa-triangle-exclamation"></i> Could not load subject name</span>`
        : `<span class="exam-link-id">ID: ${esc(item.id)}</span>`;
      return `
        <div class="exam-link-card">
          <div class="exam-link-info">
            <div class="exam-link-subject">
              <i class="fa-solid fa-book-open"></i>
              ${esc(label)}
            </div>
            ${subText}
            <div class="exam-link-url">${esc(examLink)}</div>
          </div>
          <button class="btn btn-outline exam-link-copy-btn" onclick="copyExamLink(this, '${examLink.replace(/'/g, "\\'")}')">
            <i class="fa-regular fa-copy"></i> Copy Link
          </button>
        </div>`;
    }).join('');

    examLinksLoaded = true;
  } catch (err) {
    container.innerHTML = `<p class="empty-row" style="padding:40px;text-align:center;color:var(--clr-danger)"><i class="fa-solid fa-circle-exclamation"></i> ${esc(err.message)}</p>`;
  }
}

function copyExamLink(btn, link) {
