...
/* ════════════════════════════════════════════════════════════════
   EXAM LINKS — read questions.json, build shareable links
════════════════════════════════════════════════════════════════ */
...
async function loadExamLinks() {
  ...
  try {
    // Fetch the exam index from questions.json (same folder as admin.html)
    const res = await fetch('questions.json?_=' + Date.now(), { cache: 'no-store' });
    if (!res.ok) throw new Error(`HTTP ${res.status}: Could not fetch questions.json`);

    const examIndex = await res.json();   // { "1520": "<url>", "1521": "<url>", ... }
    const entries   = Object.entries(examIndex);

    if (entries.length === 0) {
      container.innerHTML = '<div class="empty-row" style="padding:40px;text-align:center"><i class="fa-solid fa-inbox"></i> No exams found in questions.json.</div>';
      return;
    }

    // For each entry, try to fetch subject name from the question file
    const baseUrl = window.location.origin + window.location.pathname.replace('admin.html', '');
    const examCards = await Promise.all(entries.map(async ([id, url]) => {
      let subject = null;
      try {
        const sep = url.includes('?') ? '&' : '?';
        const qRes = await fetch(url + sep + '_=' + Date.now(), { cache: 'no-store' });
        if (qRes.ok) {
          const qData = await qRes.json();
          subject = qData.subject || null;
        }
      } catch (_) { /* silently ignore — subject will fall back */ }
      return { id, url, subject };
    }));

    const examLink = id => `${baseUrl}index.html?examid=${id}`;

    container.innerHTML = `
      <div class="exam-links-grid">
        ${examCards.map(({ id, url, subject }) => {
          const label = subject || `Exam ${id}`;
            ...
            <div class="exam-link-card">
              <div class="exam-link-top">
                ...
                <div class="exam-link-info">
                  <p class="exam-link-subject">${esc(label)}</p>
                  <span class="exam-link-id">ID: ${esc(id)}</span>
                </div>
              </div>
...
function copyExamLink(link, id) {
  navigator.clipboard.writeText(link).then(() => {
    const btn  = document.querySelector(`button[onclick="copyExamLink('${link}', '${id}')"]`);
