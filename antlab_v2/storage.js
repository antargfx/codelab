...
console.info('Tip: Open the console panel to see output here.');`;
  ...
  /* =============================================
     ...
     Canonical store is `files: [{name, type, content}]`.
     ============================================= */
  function createProject(name) {
    return {
      ...
      files: [
        { name: 'index.html', type: 'html', content: DEFAULT_HTML },
        { name: 'style.css',  type: 'css',  content: DEFAULT_CSS  },
        { name: 'script.js',  type: 'js',   content: DEFAULT_JS   },
      ],
      createdAt: Date.now(),
      updatedAt: Date.now(),
    };
  ...
  /* Migrate legacy {html,css,js,extraFiles} projects to the files[] model. */
  function normalizeProject(p) {
    if (!p) return p;
    if (!Array.isArray(p.files)) {
      const files = [];
      files.push({ name: 'index.html', type: 'html', content: p.html || '' });
      files.push({ name: 'style.css',  type: 'css',  content: p.css  || '' });
      files.push({ name: 'script.js',  type: 'js',   content: p.js   || '' });
      if (Array.isArray(p.extraFiles)) {
        p.extraFiles.forEach((f) => {
          if (f && f.name) files.push({ name: f.name, type: f.type || guessType(f.name), content: f.content || '' });
        });
      }
      p.files = files;
    }
  ...
  async function getAllProjects() {
    const list = usingFallback ? lsGetAll() : await idbGetAll(STORE_PROJECTS);
    return list.map(normalizeProject).sort((a, b) => b.updatedAt - a.updatedAt);
  }

  async function getProject(id) {
    const p = usingFallback
      ? (lsGetAll().find(p => p.id === id) || null)
      : (await idbGet(STORE_PROJECTS, id) || null);
