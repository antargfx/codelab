...
<div id="inspectorPanel">
  ...
  /* =============================================
     ...
     ============================================= */
  // Source HTML of the page currently shown in the preview.
  function currentHtmlSource() {
    const vfs = window._currentVFS || [];
    const page = (window.Preview && Preview.getCurrentPageName) ? Preview.getCurrentPageName() : 'index.html';
    const f = vfs.find((x) => x.name === page)
      || vfs.find((x) => /^index\.html?$/i.test(x.name))
      || vfs.find((x) => /\.html?$/i.test(x.name));
    return f ? f.content : '';
  }

  function writeStyleBack(prop, value) {
    if (!_onWriteBack || !selectedPath.length) return;
    let html = currentHtmlSource();
    if (!html) return;
  ...
  function writeAttrBack(attr, value) {
    if (!_onWriteBack || !selectedPath.length) return;
    const src = currentHtmlSource();
    if (!src) return;
    try {
      let html = injectAttrChange(src, selectedPath, attr, value);
      if (_onWriteBack) _onWriteBack(html);
