...
const App = (() => {
  ...
  function cacheDOM() {
    [
      ...
      'gridSizeSlider','exportLinksBtn','downloadAllBtn','downloadOneByOneBtn',
      // Export modal
      'exportModal','exportBackdrop','closeExportModal','exportLinksList',
      'exportCopyAllBtn','exportDownloadCsvBtn',
      // Camera
      ...
      'thresholdSlider','thresholdValue','thresholdMinus','thresholdPlus',
  ...
  function bindEvents() {
    ...
    D.exportLinksBtn.addEventListener('click', openExportModal);
    D.downloadOneByOneBtn.addEventListener('click', downloadOneByOne);
    D.loadMoreBtn.addEventListener('click', () => renderCards(true));

    /* export modal */
    D.exportBackdrop.addEventListener('click',     () => closeModal('exportModal'));
    D.closeExportModal.addEventListener('click',   () => closeModal('exportModal'));
    D.exportCopyAllBtn.addEventListener('click',   exportCopyAll);
    D.exportDownloadCsvBtn.addEventListener('click', exportDownloadCsv);
    ...
    D.thresholdSlider.addEventListener('input', () => {
      const val = parseFloat(D.thresholdSlider.value).toFixed(2);
      D.thresholdValue.textContent = val;
      Storage.set('dff_threshold', val);
      updateThresholdBadge(parseFloat(val));
      onThresholdChange();
    });

    D.thresholdMinus.addEventListener('click', () => adjustThreshold(-0.01));
    D.thresholdPlus.addEventListener('click',  () => adjustThreshold(+0.01));
  ...
  function loadSettingsValues() {
    const cfg = DRIVEFACE_CONFIG;
    const tVal = Storage.get('dff_threshold', String(cfg.defaultThreshold || 0.55));
    D.thresholdSlider.value      = tVal;
    D.thresholdValue.textContent = parseFloat(tVal).toFixed(2);
    updateThresholdBadge(parseFloat(tVal));
  ...
  function updateThresholdBadge(val) {
    if (!D.thresholdValue) return;
    D.thresholdValue.classList.remove('strict', 'loose');
    if (val >= 0.65) D.thresholdValue.classList.add('strict');
    else if (val <= 0.45) D.thresholdValue.classList.add('loose');
  }

  function adjustThreshold(delta) {
    const current = parseFloat(D.thresholdSlider.value);
    const next    = Math.round((current + delta) * 100) / 100;
    const clamped = Math.min(0.85, Math.max(0.30, next));
    D.thresholdSlider.value      = clamped;
    D.thresholdValue.textContent = clamped.toFixed(2);
    Storage.set('dff_threshold', String(clamped.toFixed(2)));
    updateThresholdBadge(clamped);
    onThresholdChange();
  }

  /**
   * Re-evaluate existing matches against the new threshold and refresh
   * the results grid instantly — no re-scan needed.
   */
  function onThresholdChange() {
    if (!S.allMatches.length) return;

    // Re-apply isMatch with the new threshold to the full match set
    // allMatches were already sorted by similarity; we just re-filter
    const cutoff = (1 - parseFloat(D.thresholdSlider.value)) * 100;
    S.filtered = S.allMatches.filter(r => r.similarity >= cutoff);

    // Also respect the confidence filter if it's not at 0
    const confVal = parseInt(D.confidenceFilter.value, 10);
    if (confVal > 0) {
      S.filtered = S.filtered.filter(r => r.similarity >= confVal);
    }

    D.resultCount.textContent = `${S.filtered.length} found`;
    showResults();
    Toast.info(`Threshold updated — ${S.filtered.length} match${S.filtered.length !== 1 ? 'es' : ''} shown.`);
  }
  ...
  async function handleFaceFile(file) {
    ...
    try {
      ...
      // Quick single-descriptor check first (fast feedback to user)
      const quickDesc = await FaceRecognition.getDescriptor(canvas);
      ...
      updateFaceProcessModal('Face found! ✓', 'Building multi-angle face profile…');
      // Build full multi-descriptor reference for robust matching
      const descs = await FaceRecognition.buildReferenceDescriptors(canvas);
      const finalDescs = descs.length > 0 ? descs : [quickDesc];

      hideFaceProcessModal();
      setFaceResult(finalDescs, dataURL, file.name);
      Toast.success(`Face captured — ${finalDescs.length} reference vector${finalDescs.length !== 1 ? 's' : ''} built!`);
    } catch (err) {
      hideFaceProcessModal();
  ...
  function setFaceResult(descriptorOrArray, dataURL, filename) {
    // Accept both a single Float32Array (legacy) and an array of Float32Arrays
    S.faceDescriptor = Array.isArray(descriptorOrArray)
      ? descriptorOrArray
      : [descriptorOrArray];
  ...
  async function capturePhoto() {
    ...
    // 1. Draw the frame to canvas FIRST (sync — no delay)
    ...
    // 2. Convert to data URL immediately (sync)
    const dataURL = canvas.toDataURL('image/jpeg', 0.9);

    // 3. Stop camera stream immediately (frees camera LED)
    if (camStream) { camStream.getTracks().forEach(t => t.stop()); camStream = null; }
    D.cameraFeed.srcObject = null;

    // 4. Close camera modal instantly, then show face processing modal
    D.cameraModal.classList.add('hidden');
    document.body.style.overflow = '';
    // Small rAF so the camera modal is visually gone before the fp modal appears
    requestAnimationFrame(() => {
      showFaceProcessModal(dataURL, 'Analysing selfie…', 'Detecting your face');
      document.body.style.overflow = 'hidden';
    });

    if (!FaceRecognition.loaded) {
      hideFaceProcessModal();
      Toast.warning('Models still loading — please try again in a moment.');
      return;
    }
    ...
    try {
      ...
      updateFaceProcessModal('Detecting face…', 'Running face detection model');
      const quickDesc = await FaceRecognition.getDescriptor(small);
      ...
      updateFaceProcessModal('Face found! ✓', 'Building multi-angle face profile…');
      const descs = await FaceRecognition.buildReferenceDescriptors(small);
      const finalDescs = descs.length > 0 ? descs : [quickDesc];

      hideFaceProcessModal();
      setFaceResult(finalDescs, dataURL, 'Camera selfie');
      Toast.success(`Selfie captured — ${finalDescs.length} reference vector${finalDescs.length !== 1 ? 's' : ''} built!`);
    } catch (err) {
      hideFaceProcessModal();
  ...
  function updateSearchBtn() {
    const ready = !!(S.folderId && S.faceDescriptor && S.faceDescriptor.length > 0 && !S.scanning);
  ...
  function renderExportList() {
    D.exportLinksList.innerHTML = '';
    S.filtered.forEach((r, i) => {
      const driveUrl  = `https://drive.google.com/file/d/${r.id}/view`;
      const directUrl = DriveAPI.viewUrl(r.id);
      const row = document.createElement('div');
      row.className = 'el-row';
      row.innerHTML = `
        <div class="el-num">${i + 1}</div>
        <div class="el-info">
          <span class="el-name" title="${Utils.esc(r.name)}">${Utils.esc(r.name || 'photo_' + r.id)}</span>
          <a class="el-url" href="${Utils.esc(driveUrl)}" target="_blank" rel="noopener">${Utils.esc(driveUrl)}</a>
        </div>
        <div class="el-score">${r.similarity}%</div>
        <button class="el-copy-btn icon-btn" title="Copy link" data-url="${Utils.esc(driveUrl)}" aria-label="Copy link">
          <i class="fa-regular fa-copy"></i>
        </button>
      `;
      row.querySelector('.el-copy-btn').addEventListener('click', function() {
        copyToClipboard(this.dataset.url, this);
      });
      D.exportLinksList.appendChild(row);
    });
  }
  ...
  function copyToClipboard(text, btn) {
    navigator.clipboard.writeText(text).then(() => {
      ...
      btn.classList.add('el-copy-btn--ok');
      setTimeout(() => {
        icon.className = 'fa-regular fa-copy';
        btn.classList.remove('el-copy-btn--ok');
      }, 1800);
  ...
  function exportCopyAll() {
    ...
    const lines = S.filtered.map((r, i) => {
      const driveUrl = `https://drive.google.com/file/d/${r.id}/view`;
      return `${i + 1}. ${r.name || 'photo_' + r.id} (${r.similarity}%)\n   ${driveUrl}`;
    });
    copyToClipboard(lines.join('\n\n'), D.exportCopyAllBtn);
    Toast.success(`${S.filtered.length} links copied to clipboard!`);
  }

  function exportDownloadCsv() {
    if (!S.filtered.length) { Toast.warning('No results to export.'); return; }
    const header = 'No.,Filename,Match %,Google Drive Link,Direct Link';
    const rows   = S.filtered.map((r, i) => {
      const driveUrl  = `https://drive.google.com/file/d/${r.id}/view`;
      const directUrl = DriveAPI.viewUrl(r.id);
      // CSV-safe: wrap fields containing commas/quotes in double-quotes
      const esc = v => `"${String(v).replace(/"/g, '""')}"`;
      return [i + 1, esc(r.name || 'photo_' + r.id), r.similarity + '%', esc(driveUrl), esc(directUrl)].join(',');
    });
    const csv  = [header, ...rows].join('\n');
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const a    = document.createElement('a');
    a.href     = URL.createObjectURL(blob);
    a.download = `my_photos_${Date.now()}.csv`;
    a.click();
    setTimeout(() => URL.revokeObjectURL(a.href), 10000);
    Toast.success(`CSV with ${S.filtered.length} links downloaded!`);
  }

  /* ══════════════════════════════════════════════════════
     DOWNLOAD ONE BY ONE
     Downloads each matched file sequentially as a separate
     browser download — no ZIP, each file saves individually.
  ══════════════════════════════════════════════════════ */
  async function downloadOneByOne() {
    if (!S.filtered.length) { Toast.warning('No photos to download.'); return; }
    if (S._downloadingOneByOne) return;

    S._downloadingOneByOne = true;
    S._cancelOneByOne = false;

    const total  = S.filtered.length;
    const origHTML = D.downloadOneByOneBtn.innerHTML;
    D.downloadOneByOneBtn.innerHTML = `<i class="fa-solid fa-spinner fa-spin"></i> 0 / ${total}`;
    D.downloadOneByOneBtn.disabled  = true;

    // Add a cancel button temporarily next to it
    const cancelOOO = document.createElement('button');
    cancelOOO.className = 'btn btn-ghost btn-sm';
    cancelOOO.innerHTML = '<i class="fa-solid fa-stop"></i>';
    cancelOOO.title = 'Stop downloads';
    cancelOOO.addEventListener('click', () => { S._cancelOneByOne = true; });
    D.downloadOneByOneBtn.insertAdjacentElement('afterend', cancelOOO);

    Toast.info(`Starting ${total} individual downloads…`);

    let done = 0;
    for (const file of S.filtered) {
      if (S._cancelOneByOne) break;

      try {
        const url  = DriveAPI.viewUrl(file.id);
      ...
      D.downloadOneByOneBtn.innerHTML = `<i class="fa-solid fa-spinner fa-spin"></i> ${done} / ${total}`;

      // Small gap between downloads so the browser isn't overwhelmed
      await Utils.sleep(400);
    }

    cancelOOO.remove();
    D.downloadOneByOneBtn.innerHTML = origHTML;
    D.downloadOneByOneBtn.disabled  = false;
    S._downloadingOneByOne = false;
    S._cancelOneByOne      = false;

    if (done === total) {
      Toast.success(`All ${total} photos downloaded individually!`);
    } else {
      Toast.warning(`Stopped after ${done} of ${total} downloads.`);
    }
  }
