...
export function initInspector() {
  ...
  bus.on(EVT.DOM_SNAPSHOT, (m) => { if (m && m.tree) { domTree = m.tree; renderDom(); if (selected) revealInTree(selected.id); } });
  bus.on(EVT.PICK_RESULT, (descr) => { selected = descr; renderStyles(); revealInTree(descr.id); });
...
/** Find the chain of element ids from root down to the given id. */
function pathToId(node, id, acc = []) {
  if (!node) return null;
  if (node.id === id) return [...acc, id];
  if (node.children) {
    for (const c of node.children) {
      if (c.type !== 'el') continue;
      const found = pathToId(c, id, node.id ? [...acc, node.id] : acc);
...
/** Expand ancestors, highlight, and scroll the selected element into view. */
function revealInTree(id) {
  if (!domTree || !id) return;
  const chain = pathToId(domTree, id);
  if (chain) { for (const aid of chain) collapsed.delete(aid); }
  renderDom();
  highlightInTree(id);
  const line = $('#inspectorDom')?.querySelector(`.dom-line[data-id="${id}"]`);
...
function renderStyles() {
  ...
  host.appendChild(htmlSnippetSection());
...
/** Show the clicked element's own HTML (the "clicked part" code). */
function htmlSnippetSection() {
  const s = el('div', { class: 'styles-section' });
  s.appendChild(el('h4', { text: 'Selected element' }));
  const pre = el('pre', { class: 'html-snippet' });
  pre.appendChild(el('code', { text: selected.outerHTML || `<${selected.tag}>` }));
  s.appendChild(pre);
  return s;
}
