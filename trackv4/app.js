...
async function syncNow() {
  // Guard: user may not be logged in yet
  if (!state.user || !state.user.uid) {
    console.warn('[Spendly] sync skipped — no authenticated user');
    return;
  }
...
function applyTheme() {
  try {
    ...
    console.warn('[Spendly Theme Error]', err);
  }
...
function startSpendlyApp() {
  try {
    if (window.__SPENDLY_STARTED__) return;
    window.__SPENDLY_STARTED__ = true;
    waitForChartJS(bootstrap);
  } catch (err) {
    console.error('[Spendly Startup Error]', err);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startSpendlyApp);
} else {
  startSpendlyApp();
}
