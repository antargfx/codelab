...
const App = (() => {
  ...
  async function capturePhoto() {
    ...
    // 3. Stop camera stream (sync — frees camera LED immediately)
    if (camStream) { camStream.getTracks().forEach(t => t.stop()); camStream = null; }
    D.cameraFeed.srcObject = null;

    // 4. Close camera modal + show face processing modal in the SAME paint frame.
    //    We do NOT call closeModal() (which sets overflow:'') because
    //    showFaceProcessModal() will set overflow:'hidden' right after.
    D.cameraModal.classList.add('hidden');
    showFaceProcessModal(dataURL, 'Analysing selfie…', 'Detecting your face');
    // body overflow stays 'hidden' — no flicker between modals
