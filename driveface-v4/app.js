...
const App = (() => {
  ...
  function closeCamera() {
    // Instant hide — no classList animation delay
    D.cameraModal.style.display = 'none';
    document.body.style.overflow = '';
    // Defer stream teardown so it doesn't block the repaint
    const streamToStop = camStream;
    camStream = null;
    setTimeout(() => {
      if (streamToStop) streamToStop.getTracks().forEach(t => t.stop());
      D.cameraFeed.srcObject = null;
      D.cameraModal.style.display = '';  // restore for next open
      D.cameraModal.classList.add('hidden'); // keep in sync with hidden class
    }, 0);
  }
  ...
  async function capturePhoto() {
    ...
    // 2. Encode to data URL (sync)
    const dataURL = canvas.toDataURL('image/jpeg', 0.9);

    // 3. Kill modal-box animation so no CSS frame is held,
    //    then force display:none directly — instant, no class toggle delay.
    const box = D.cameraModal.querySelector('.modal-box');
    if (box) box.style.animation = 'none';
    D.cameraModal.style.display = 'none';

    // 4. Show face processing modal in the very next paint (same sync block)
    showFaceProcessModal(dataURL, 'Analysing selfie…', 'Detecting your face');

    // 5. Defer ALL stream teardown — stopping tracks blocks the thread
    //    on many browsers/devices for 100–400ms. Do it after the paint.
    const streamToStop = camStream;
    camStream = null;
    setTimeout(() => {
      if (streamToStop) streamToStop.getTracks().forEach(t => t.stop());
      D.cameraFeed.srcObject = null;
      D.cameraModal.style.display = '';   // restore for next open
      if (box) box.style.animation = '';  // restore animation for next open
    }, 0);
