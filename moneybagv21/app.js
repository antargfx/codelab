...
function updateBankUI() {
  ...
  setElText('bank-balance-peek', text);
}
...
function renderDashboard() {
  ...
  setElText('bank-balance-peek', hidden ? '••••' : fmt(bankBalance));
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ── Bank Savings collapse (peek under balance card) ──
  const bankCollapse = document.getElementById('bank-collapse');
  const balanceCard = document.getElementById('balance-card');
  const bankPeek = document.getElementById('bank-peek');

  function toggleBankCollapse() {
    bankCollapse?.classList.toggle('open');
  }
  ...
  bankPeek?.addEventListener('click', e => {
    e.stopPropagation();
    toggleBankCollapse();
  });

  document.addEventListener('click', e => {
    if (!bankCollapse?.classList.contains('open')) return;
    if (e.target.closest('#bank-collapse') || e.target.closest('#balance-card')) return;
    bankCollapse.classList.remove('open');
  });
