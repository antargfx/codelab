...
function isMoneybagRecord(r) {
  return !!r && !r.deleted && (r.type === 'add' || r.type === 'spend') && r.category !== 'Bank';
}

function moneybagRecordsList() {
  return records.filter(isMoneybagRecord);
}

function moneybagAddsList() {
  return moneybagRecordsList().filter(r => r.type === 'add');
}

function moneybagSpendsList() {
  return moneybagRecordsList().filter(r => r.type === 'spend');
}
  ...
  try {
    ...
    recordUnsub = onSnapshot(
      ...
      snap => {
        ...
        const pendingUpserts = pendingSync
          .filter(op => op.type === 'upsert' && op.record && op.record.id)
          .map(op => op.record);

        remoteRecords.forEach(r => {
          if (!r || !r.id || pendingDeleteIds.has(r.id)) return;
          merged[r.id] = r;
        });

        pendingUpserts.forEach(r => {
        ...
        const syncDetail = document.getElementById('sync-detail');
        if (syncDetail) syncDetail.textContent = 'Live synced';
        setSyncStatus('idle');
      ...
      err => {
        ...
        const syncDetail = document.getElementById('sync-detail');
        if (syncDetail) syncDetail.textContent = 'Sync error';
      }
    );
...
/* ══════════════ CALCULATIONS ══════════════ */
function calcStats() {
  const active = moneybagRecordsList();
...
function renderDashboard() {
  ...
  const recent = moneybagRecordsList().sort((a,b) => b.createdAt - a.createdAt).slice(0, 5);
...
function renderInsights(stats) {
  ...
  if (stats.weeklySpent > 0) {
    ...
    const lastWeekSpent = moneybagSpendsList().filter(r => r.createdAt >= now - 2 * weekMs && r.createdAt < now - weekMs)
      .reduce((s, r) => s + Number(r.amount), 0);
    if (lastWeekSpent > 0) {
  ...
  const oldest = Math.min(...moneybagSpendsList().map(r => r.createdAt));
  if (isFinite(oldest)) {
...
function renderBarChart() {
  ...
  if (activePeriod === 'week') {
    for (let i = 6; i >= 0; i--) {
      ...
      incomeData.push(moneybagAddsList().filter(r => r.createdAt >= dayStart && r.createdAt < dayEnd).reduce((s,r) => s+Number(r.amount), 0));
      expenseData.push(moneybagSpendsList().filter(r => r.createdAt >= dayStart && r.createdAt < dayEnd).reduce((s,r) => s+Number(r.amount), 0));
    }
  } else {
    for (let i = 5; i >= 0; i--) {
      ...
      incomeData.push(moneybagAddsList().filter(r => r.createdAt >= mStart && r.createdAt < mEnd).reduce((s,r) => s+Number(r.amount), 0));
      expenseData.push(moneybagSpendsList().filter(r => r.createdAt >= mStart && r.createdAt < mEnd).reduce((s,r) => s+Number(r.amount), 0));
    }
  }
...
function renderLineChart() {
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    const mStart = m.getTime();
    labels.push(m.toLocaleDateString('en-US', { month: 'short' }));
    const inc = moneybagAddsList().filter(r => r.createdAt >= mStart && r.createdAt < mEnd).reduce((s,r) => s+Number(r.amount), 0);
    const exp = moneybagSpendsList().filter(r => r.createdAt >= mStart && r.createdAt < mEnd).reduce((s,r) => s+Number(r.amount), 0);
...
function renderCategoryLegend() {
  ...
  if (!el) return;
  const cats = [...new Set(moneybagSpendsList().map(r => r.category))];
...
function renderSmartReview(stats) {
  ...
  const lastWeekSpent = moneybagSpendsList().filter(r => r.createdAt >= now - 2 * weekMs && r.createdAt < now - weekMs).reduce((s, r) => s + Number(r.amount), 0);
  const lastMonthSpent = (() => {
    ...
    return moneybagSpendsList().filter(r => r.createdAt >= lmS && r.createdAt < lmE).reduce((s, r) => s + Number(r.amount), 0);
  })();
  ...
  const cats = {};
  moneybagSpendsList().forEach(r => { cats[r.category] = (cats[r.category] || 0) + Number(r.amount); });
...
function updateFilterCategories() {
  ...
  if (!sel) return;
  const cats = [...new Set(moneybagRecordsList().map(r => r.category))].sort();
...
function applyRecordFilters() {
  ...
  let filtered = moneybagRecordsList();
...
function exportCSV() {
  ...
  const rows = moneybagRecordsList().map(r =>
