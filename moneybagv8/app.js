...
/* ══════════════ FIREBASE CRUD ══════════════ */
function mergeRemoteAndPending(remoteRecords = []) {
  ...
  for (const op of pendingSync) {
    if (op.type === 'upsert' && op.record && op.record.id) {
      merged.set(op.record.id, op.record);
...
function applyRemoteRecords(remoteRecords = []) {
  records = mergeRemoteAndPending(remoteRecords);
  saveLocal();
  ...
  setSyncStatus(pendingSync.length ? 'pending' : 'idle');
}
...
async function syncFromFirestore() {
  ...
  try {
    ...
    recordsUnsub = onSnapshot(col, snapshot => {
      const liveRecords = [];
      snapshot.forEach(d => liveRecords.push(d.data()));
      applyRemoteRecords(liveRecords);
      const syncDetail = document.getElementById('sync-detail');
      if (syncDetail) syncDetail.textContent = 'Last synced: just now';
    }, err => {
      console.error('Live records sync error:', err);
...
async function flushPendingSync() {
  if (!fbAvailable || !currentUser || !navigator.onLine || !pendingSync.length) return;
...
async function upsertRecord(record) {
  ...
  pendingSync = pendingSync.filter(op => !(op.type === 'delete' && op.id === payload.id) && !(op.type === 'upsert' && op.record?.id === payload.id));
  saveLocal();

  try {
    if (fbAvailable && currentUser && navigator.onLine) {
      const ref = doc(db, 'users', currentUser.uid, 'moneybagRecords', payload.id);
      await setDoc(ref, payload);
    } else {
      pendingSync.push({ type: 'upsert', record: payload });
    }
  } catch (e) {
    console.error('Upsert sync error:', e);
    pendingSync.push({ type: 'upsert', record: payload });
  }
...
async function deleteRecord(id) {
  ...
  pendingSync = pendingSync.filter(op => !(op.type === 'upsert' && op.record?.id === id) && !(op.type === 'delete' && op.id === id));
  saveLocal();

  try {
    if (fbAvailable && currentUser && navigator.onLine) {
      const ref = doc(db, 'users', currentUser.uid, 'moneybagRecords', id);
      await deleteDoc(ref);
    } else {
