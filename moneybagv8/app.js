...
async function deleteRecord(id) {
  const removed = records.find(r => r.id === id) || null;
  records = records.filter(r => r.id !== id);
  pendingSync = pendingSync.filter(op => !(op.type === 'delete' && op.id === id));
  pendingSync = pendingSync.filter(op => !(op.type === 'upsert' && op.record?.id === id));
  saveLocal();
