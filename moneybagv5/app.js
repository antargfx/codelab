...
let db = null, auth = null, storage = null, fbAvailable = false;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('photo-file-input')?.addEventListener('change', async e => {
    ...
    if (!fbAvailable || !currentUser || !storage) {
    ...
    try {
      ...
      const path = `profilePhotos/${currentUser.uid}/avatar-${Date.now()}-${cleanName}`;
      const ref = storageRef(storage, path);
      await uploadBytes(ref, file, { contentType: file.type || 'image/jpeg' });
      userPhoto = await getDownloadURL(ref);
      saveProfileLocal();
      await saveProfileState();
      updateProfileUI();
